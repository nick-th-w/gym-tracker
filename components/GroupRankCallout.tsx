...
import { getDailyActivityLine, formatTimeAgo } from '@/lib/activityLines'
...
export default async function GroupRankCallout({ userId }: { userId: string }) {
  ...
  const [{ data: leaderboard }, { data: activity }] = await Promise.all([
    supabase.rpc('get_group_leaderboard', { p_group_id: membership.group_id }),
    supabase.rpc('get_group_activity', { p_group_id: membership.group_id }),
  ])
  ...
  const activityRow = ((activity ?? []) as GroupActivity[])[0]
  const activityLine = activityRow
    ? getDailyActivityLine({
        name: activityRow.last_user_id === userId ? 'You' : activityRow.last_display_name,
        time: formatTimeAgo(activityRow.last_logged_at),
        todayCount: activityRow.today_count,
      })
    : null
  ...
  return (
    <Link
      ...
      className="mt-3 bg-card border border-border rounded-2xl px-4 py-3.5 flex flex-col gap-2 active:scale-[0.98] transition-all"
    >
      ...
      {activityLine && (
        <p className="text-amber-400 text-xs border-t border-border pt-2">{activityLine}</p>
      )}
    </Link>
  )
}
