...
function LineChart({ data }: { data: { label: string; value: number }[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
      ...
      {data.map((d, i) => (
        <circle key={i} cx={px(i)} cy={py(d.value)} r="3" fill="#8cc63f" />
      ))}
...
function BarChart({ data }: { data: { label: string; value: number }[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
      <text x={P.l - 5} y={P.t + 4} textAnchor="end" fill="#8e8e93" fontSize="9">
        {maxV >= 1000 ? `${(maxV / 1000).toFixed(1)}t` : `${Math.round(maxV)}`}
      </text>
      <line x1={P.l} y1={P.t} x2={P.l} y2={P.t + cH} stroke="#525254" strokeWidth="0.5" />
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            <rect x={bX} y={bY} width={bW} height={bH || 1} fill={d.value > 0 ? '#8cc63f' : '#3a3a3c'} rx="2" />
            <text x={bX + bW / 2} y={H - 5} textAnchor="middle" fill="#8e8e93" fontSize="8">{d.label}</text>
          </g>
        )
      })}
    </svg>
...
export default async function ProgressPage({ searchParams }: { searchParams?: { muscle?: string; exercise?: string } }) {
  ...
  // Exercises for filters
  const { data: allExercises } = await supabase
    .from('exercises').select('id, name, muscle_groups').order('name')

  const allMuscles = [...new Set(
    (allExercises ?? []).flatMap(e => e.muscle_groups as string[])
  )].sort()
  ...
  for (const we of allWEs ?? []) {
    (weByExercise[we.exercise_id] ??= []).push({ weId: we.id, workoutId: we.workout_id })
  }
  ...
  // ── Exercise progress chart data (if exercise selected) ──────────────────
  ...
  for (const we of allWEs ?? []) {
    ...
    if (!date) continue
    const week = getMondayKey(date)
    for (const s of setsByWE[we.id] ?? []) {
      volumeByWeek[week] = (volumeByWeek[week] ?? 0) + s.weight_kg * s.reps
    }
  }
  const weeklyData = Array.from({ length: 8 }, (_, i) => {
    ...
    return { label: fmtDate(key).split(' ')[0], value: volumeByWeek[key] ?? 0 }
  })
  ...
  return (
    <div className="px-4 pt-8 pb-6">
      ...
      <p className="text-secondary-text text-sm mb-5">Track your lifts and volume over time</p>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-3 scrollbar-hide">
        ...
        {allMuscles.map(m => (
          <Link
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap shrink-0 ${activeMuscle === m ? 'bg-primary text-white' : 'bg-card text-secondary-text border border-border'}`}
          >
            {m}
          </Link>
        ))}
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-6 scrollbar-hide">
        {activeMuscle || activeExerciseId ? (
          <Link
            href={activeMuscle ? `/progress?muscle=${encodeURIComponent(activeMuscle)}` : '/progress'}
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap shrink-0 ${!activeExerciseId ? 'bg-success text-white' : 'bg-card text-secondary-text border border-border'}`}
          >
            All exercises
          </Link>
        ) : null}
        {filteredExercises.map(e => (
          <Link
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap shrink-0 ${activeExerciseId === e.id ? 'bg-success text-white' : 'bg-card text-secondary-text border border-border'}`}
          >
            {e.name}
          </Link>
        ))}
      ...
      {!hasAnyData ? (
        ...
        <div>
          ...
          {allTimeBest && (
            <div className="bg-success/15 border border-success/30 rounded-xl p-4 mb-5">
        ...
        <div>
          ...
          {!activeMuscle && (
            <div className="bg-card border border-border rounded-xl p-4">
              <p className="text-secondary-text text-xs uppercase tracking-wide mb-3">Weekly volume — last 8 weeks</p>
